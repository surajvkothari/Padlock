...
def encrypt(passKey, cipher, dataformat, plaintext=None, filename=None, filepath=None, cipherMode=None):
    ...
    if dataformat == "Messages":
        if cipher == "Caesar Cipher":
            guide_data, encryptedData = caesarCipher.encrypt(plaintext=plaintext, passKey=passKey,
                dataformat=dataformat, cipherMode=cipherMode)
        elif cipher == "Vigenere Cipher":
            guide_data, encryptedData = vigenereCipher.encrypt(plaintext=plaintext, passKey=passKey,
                dataformat=dataformat, cipherMode=cipherMode)
        elif cipher == "DES Cipher":
            guide_data, encryptedData = DES_Cipher.encrypt(plaintext=plaintext, passKey=passKey,
                dataformat=dataformat)
        elif cipher == "Triple DES Cipher":
            guide_data, encryptedData = DES_Cipher.encrypt(plaintext=plaintext, passKey=passKey,
                dataformat=dataformat, isTripleDES=True)

    # If the data format is either a file or an image, a filename argument will need to be passed.
    else:
        if cipher == "Caesar Cipher":
            guide_data, encryptedData = caesarCipher.encrypt(filename=filename, filepath=filepath,
                passKey=passKey, dataformat=dataformat, cipherMode=cipherMode)
        elif cipher == "Vigenere Cipher":
            guide_data, encryptedData = vigenereCipher.encrypt(filename=filename, filepath=filepath,
                passKey=passKey, dataformat=dataformat, cipherMode=cipherMode)
        elif cipher == "DES Cipher":
...
def decrypt(passKey, cipher, dataformat, ciphertext=None, filename=None, filepath=None, cipherMode=None):
    ...
    if dataformat == "Messages":
        if cipher == "Caesar Cipher":
            guide_data, decryptedData = caesarCipher.decrypt(ciphertext=ciphertext, passKey=passKey,
                dataformat=dataformat, cipherMode=cipherMode)
        elif cipher == "Vigenere Cipher":
            guide_data, decryptedData = vigenereCipher.decrypt(ciphertext=ciphertext, passKey=passKey,
                dataformat=dataformat, cipherMode=cipherMode)
        elif cipher == "DES Cipher":
            guide_data, decryptedData = DES_Cipher.decrypt(ciphertext=ciphertext, passKey=passKey,
                dataformat=dataformat)
        elif cipher == "Triple DES Cipher":
            guide_data, decryptedData = DES_Cipher.decrypt(ciphertext=ciphertext, passKey=passKey,
                dataformat=dataformat, isTripleDES=True)
    ...
    # If the data format is either a file or an image, a filename argument will need to be passed.
        ...
        if cipher == "Caesar Cipher":
            guide_data, decryptedData = caesarCipher.decrypt(filename=filename, filepath=filepath,
                passKey=passKey, dataformat=dataformat, cipherMode=cipherMode)
        elif cipher == "Vigenere Cipher":
            guide_data, decryptedData = vigenereCipher.decrypt(filename=filename, filepath=filepath,
                passKey=passKey, dataformat=dataformat, cipherMode=cipherMode)
        elif cipher == "DES Cipher":
